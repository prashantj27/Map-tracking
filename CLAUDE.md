...
8. **Uploaded project images live in a separate IndexedDB** (`src/lib/imageStore.ts`, database `ProjectImages`), never in `MapDatabase`. `db.delete()` during a reseed must never destroy user uploads. Link images by `Project_Code`.
9. **Projects are their own GIS layer (PRJ), independent of facilities.** Each project renders a dedicated clustered marker at its own coordinates (`ProjectLayer` inside `MapView`), with a project-centric popup + detail modal. Do **not** reference facilities/NCOEs in project UI. The build-time `Parent_Facility_*` fields remain in the data but are no longer used for display. `Project_Code` is an internal id — never a UI filter. Facility and PRJ layers toggle/filter independently.
10. **Permission-gated actions go through `src/lib/permissions.ts`.** There is no auth layer yet; gate future RBAC there, not at call sites.
11. **Infra-type strings emitted by `classifyInfra` must all be keys in `INFRA_CONFIG`** (`src/lib/projects.ts`) so icons/colors/ordering stay consistent.
