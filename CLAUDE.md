...
9. **Projects associate to a parent facility, computed once at build time** in `scripts/convert_projects.cjs` (largest NCOE of the state, else the state's largest facility). Phase-1 "Show on Map" uses that parent; the single `USE_PROJECT_COORDINATES` flag in `src/lib/projects.ts` is the only switch needed to move to real project coordinates. `Project_Code` is an internal id — never a UI filter.
