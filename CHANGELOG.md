...
### Added
- **Real project GPS coordinates.** 262 projects get exact `Latitude`/`Longitude` + `Google_Maps_URL` from a "Project Locations" source (coordinates parsed from each row's Google Maps Link, cross-checked against its own lat/lng columns). 28 projects with no confirmed location (the source's own "Projects Without Coordinates" list — verified to *exactly* match the rows lacking a link, or the pipeline aborts) have their coordinates **cleared** rather than left stale, so they get no map marker until real data arrives instead of a misleading approximate one. New `scripts/build_project_gps_overrides.cjs` + `apply_project_gps_coordinates.cjs` (same provenance pattern as the facility coordinate overlay); `Project` gained `Google_Maps_URL`.
...
### Changed
- **"Without GPS Images" now also flags missing coordinates.** `projectMatchesStatusFilter` (`lib/projects.ts`) matches a project if it has no confirmed coordinate *or* has coordinates but zero uploaded gallery photos (previously photo-count only). A project without coordinates still can't get a map marker (there's nothing to plot), so it stays reachable by name via search — flying/opening its popup is a no-op until a real location is set — but the filter's own logic now correctly recognises it as needing GPS attention, verified end-to-end (state-scoped marker count changes correctly when a project gains both a coordinate and an uploaded photo).
