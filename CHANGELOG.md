...
### Changed
- **Premium pin icon set.** Every marker (facilities + projects) now uses one consistent teardrop frame with a white type glyph — building (RC), trophy (NCOE), runner (STC), columns (EXT), participant (KIC), institution+star (KISCE), handshake (AKH), crane (Projects) — via the shared `MapPinGraphic`. The facility-type palette (`FACILITY_CONFIG`) was updated to the reference colours (NCOE green, STC orange, EXT purple, KIC pink, AKH gold; Projects navy), which flows to the legend, popups, search dots and report-card chips for one consistent design. Replaces the earlier acronym labels and the STC/EXT/AKH silhouette shapes.
