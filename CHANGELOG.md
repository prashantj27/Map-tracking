...
### Changed
- **Premium pin icon set.** Markers now use a consistent teardrop frame with a white type glyph — building (RC), trophy (NCOE), runner (STC), columns (EXT), institution+star (KISCE), handshake (AKH), crane (Projects) — via the shared `MapPinGraphic`. The facility-type palette (`FACILITY_CONFIG`) was updated to the reference colours (NCOE green, STC orange, EXT purple, AKH gold; Projects navy), which flows to the legend, popups, search dots and report-card chips. **KIC keeps its previous look** — the classic purple (`#8e24aa`) teardrop with the "KIC" acronym. Replaces the earlier acronym labels (for all other types) and the STC/EXT/AKH silhouette shapes.
