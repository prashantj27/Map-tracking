...
- **GPS coordinate overlay** (`scripts/build_project_gps_overrides.cjs` + `apply_project_gps_coordinates.cjs`, same two-stage provenance pattern as the facility coordinate overlay, and equally surgical — an override object only carries the fields it means to change): matches an external "Project Locations" sheet to `sai_projects.json` by `Project_Code`, parses each row's Google Maps Link for the exact `Latitude`/`Longitude` (cross-checked against the sheet's own lat/lng columns — the script aborts on any disagreement), and sets `Google_Maps_URL` + `Without_GPS_Images: false`. Rows with no link (their own "Projects Without Coordinates" sheet — verified to be *exactly* that subset, or the script aborts) get **no coordinate fields in their override at all** — whatever location the project already has (e.g. an earlier interim/approximate one) is left completely untouched — and are instead flagged `Without_GPS_Images: true`, a permanent record that real GPS-verified site photos are still needed for that project.
