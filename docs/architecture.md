...
- **Filtered-facility count + Excel export**: whenever a facility filter (State and/or Facility Type) is active, a strip appears below the search box showing the **matching facility count** and a green **Excel** button. It downloads a real `.xlsx` of exactly those facilities — columns *S.No / Facility Type / Facility Name / Coordinates / Address* — via `lib/exportFacilities.ts`, which **dynamic-imports `xlsx` (SheetJS)** so the ~425 kB library is code-split into its own chunk and only fetched on the first export (never in the initial bundle).
