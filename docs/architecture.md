...
- **Project-centric flow**: clicking a PRJ marker opens `ProjectPopup` (name, state, district if present, status, **progress %**, Directions, **Birdeye** — same as the facility popup: closes the popup and flies straight down to the project at `BIRDEYE_ZOOM` — and `View Project Details →` — no facility/NCOE reference) → `ProjectDetailModal` (Overview — incl. a **progress bar + %** — / Financials / Timeline / Gallery / Documents / Remarks). "Show on Map" (search result / future list) switches the type selector to Projects, flies to the PRJ marker and opens its popup.
- **Status + progress overlay** (`scripts/build_project_status_overrides.cjs` + `apply_project_status.cjs`, same surgical two-stage pattern): reads a progress workbook's "All Projects" sheet (`Project Code`, `Progress (%)` 0–1, `Status`) and sets each project's `Status` + `Progress` (0–100). The mapping is two small, isolated functions (easy to change): `mapStatus` collapses the source's statuses to the platform's three — `Completed` / `Cancelled` / everything-else → `In Progress`; `deriveStatusProgress` forces `Completed ⇒ 100`, keeps the real % for `In Progress`, and leaves `Cancelled` with no progress. `Without_GPS_Images` is **never touched** here — it stays independent of status. Re-run both scripts when an updated workbook arrives. Currently 270 Completed / 76 In Progress / 27 Cancelled.
- **Project filters** (shown only while the type selector is set to Projects): a **Status** row (`In Progress` / `Completed` / `Cancelled`, single-select — click again to clear; `p.Status === key`) **plus an independent, dashed "Without GPS Images" toggle** on its own line. The two **combine (AND)**, so e.g. *Completed + Without GPS Images* = the 33 completed projects still needing GPS-verified photos. "Without GPS Images" (`projectIsWithoutGps` in `lib/projects.ts`) shows the projects **flagged** `Without_GPS_Images: true` by the coordinate overlay that the user has **not** since marked **"Coordinates available"** — a per-project confirmation stored in `coordConfirmations` inside the reseed-surviving `ProjectImages` DB (`lib/imageStore.ts`), sticky across data refreshes, fed into the filter reactively via `useLiveQuery`. Marking a project "Coordinates available" from its **Gallery tab** (`CoordinatesAvailableToggle` in `ProjectDetailModal`) removes it from that filter; reversible.
...
- **Search** typeahead over facility name/city/district and project name/**code**/state/district (min 2 chars, top 6 each, tagged `PRJ`; the project's `Project_Code` shows in the result meta) with fly-to + popup-open on select; results drop upward since the panel is bottom-anchored. The search box also hosts the **satellite-view toggle** (satellite icon, before the drag handle) — see the Map section above.
- **Filtered count + Excel export**: whenever a filter is active, a strip appears below the search box showing the **matching count** and a green **Excel** button that downloads a real `.xlsx` of exactly the filtered rows (`lib/exportExcel.ts`). It adapts to the active layer: **facilities** (State/Facility-Type filters) export *S.No / Facility Type / Facility Name / Coordinates / Address*; the **Projects** layer exports *S.No / Project Code / Project Name / Infrastructure Type / Status / Progress / State / District / Coordinates / GPS Images Pending* (respecting the combined Status + Without-GPS filters). `xlsx` (SheetJS) is **dynamic-imported**, so the ~425 kB library is code-split into its own chunk and only fetched on the first export (never in the initial bundle).
...
- **Draggable** (desktop): one grip handle (`DragHandle.tsx`, in the search box) drags the whole panel — search, State, chips, and Project Status all move together (`lib/useDraggable.ts`); position is clamped to stay fully visible and persisted per-browser in `localStorage`. Double-click the handle to reset to the default bottom-left spot.
- **Mobile / tablet (`useIsMobileOrTablet()`, ≤1024px, `lib/useMediaQuery.ts`)**: the panel is compact, moves to the **top**, isn't draggable, and the search results drop **down**. It minimizes via a **glowing, gently-bobbing pull-tab** (`.mfp-minimize-arrow`) protruding at the panel's bottom edge (`prefers-reduced-motion` disables the animation). The Facility Type control swaps from the icon-chip row to a compact **icon dropdown** (`TypeSelectDropdown.tsx` — the same `typeSelection` state; a native `<select>` can't render per-type `TypeIcon`s). Picking a search result **auto-minimizes** the panel to two circular buttons (reopen search + the zoom-to-map reset, when zoomed in); tap the search button to expand again.
...
A full-height side panel (desktop) opened by clicking a state (map or panel link): an overview line (rank among states, **facility and project counts**), facility mix, trainees + seat-utilization bar (only where sanctioned strength is recorded), clickable discipline chips that list the offering facilities, KISCE funds by financial year, KISCE staffing (in-post / sanctioned / vacant), the top 5 facilities by trainees, and — at the end — a **Projects** section listing every project in the state (matched by `State`). Every facility row flies the map to that facility and opens its popup; every project row closes the report and flies to that project in **birdeye view** (Projects layer shown, aerial close-up at `BIRDEYE_ZOOM`). **On mobile/tablet (≤1024px) it becomes a bottom sheet**: slides up from the bottom, half-screen (58vh) by default with a scrollable body, and a grab handle toggles it to near-full (92vh).
