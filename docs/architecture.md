...
- **Project-centric flow**: clicking a PRJ marker opens `ProjectPopup` (name, state, district if present, status, Directions, **Birdeye** — same as the facility popup: closes the popup and flies straight down to the project at `BIRDEYE_ZOOM` — and `View Project Details →` — no facility/NCOE reference) → `ProjectDetailModal` (Overview / Financials / Timeline / Gallery / Documents / Remarks). "Show on Map" (search result / future list) switches the type selector to Projects, flies to the PRJ marker and opens its popup.
- **Project Status filter**: a status-chip row (`In Progress` / `Completed` / `Cancelled` / `Without GPS Images`) that appears only while the type selector is set to Projects. "Without GPS Images" (`projectMatchesStatusFilter` in `lib/projects.ts`) shows exactly the projects **explicitly flagged** `Without_GPS_Images: true` by the source data (the GPS coordinate overlay above — 111 today) that the user has **not** since marked **"Coordinates available"**. That mark is a per-project confirmation stored in `coordConfirmations` inside the reseed-surviving `ProjectImages` DB (`lib/imageStore.ts`), so it sticks across data refreshes just like uploaded images; App feeds the set of confirmed codes into the filter reactively via `useLiveQuery`. Marking a flagged project "Coordinates available" from its **Gallery tab** (`CoordinatesAvailableToggle` in `ProjectDetailModal`) removes it from the filter (and from the map when the filter is active); the mark is reversible.
