...
- **Project Status filter**: a status-chip row (`In Progress` / `Completed` / `Cancelled` / `Without GPS Images`) that appears only while the type selector is set to Projects. "Without GPS Images" matches projects with zero uploaded gallery photos (`projectCodesWithImages()` in `lib/imageStore.ts` — an index-only query, no blobs loaded), a practical proxy for site-verification status until a dedicated geotag field exists.
